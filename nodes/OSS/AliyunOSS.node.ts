...
import { aliyunApiRequest } from './GenericFunctions';

// TODO just quick finish functions 
import OSS, { ListV2ObjectsQuery } from 'ali-oss';
...
export class AliyunOSS implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Sends data to Aliyun OSS',
		defaults: {
			name: 'Aliyun OSS',
		},
		inputs: ['main'],
    ...
    const credentials = await this.getCredentials('aliyun') as IDataObject;
    ...
    for (let i = 0; i < items.length; i++) {
			...
			try {
				if (resource === 'bucket') {
					...
					if (operation === 'create') {
						// const credentials = await this.getCredentials('aliyun');
            ...
            */
						...
						responseData = await aliyunApiRequest.call(
							this,
							`${name}`,
							'PUT',
							'',
            ...
            if (returnAll) {
              response = await oss.listBuckets({}) as any;
							/*
              responseData = await aliyunApiRequest.call(
								this,
              ...
              */
              ...
              response = await oss.listBuckets({
                'max-keys': qs.limit as number,
              }) as any;

							// responseData = responseData.slice(0, qs.limit);
						}
            responseData = response.buckets;
					...
					if (operation === 'search') {
						...
						responseData = await aliyunApiRequest.call(this, `${bucketName}.s3`, 'GET', '', '', {
							location: '',
						});
          ...
          if (operation === 'getAll') {
            ...
            */
            ...
            if (Array.isArray(responseData)) {
							responseData = responseData.filter(
								(e: OSS.ObjectMeta) =>
									e.name.endsWith('/') && e.size === 0 && e.name !== options.folderKey,
							);
            ...
            */
					...
					if (operation === 'download') {
						...
						let region = await aliyunApiRequest.call(this, `${bucketName}.s3`, 'GET', '', '', {
							location: '',
						});

						region = region.LocationConstraint._;

						const response = await aliyunApiRequest.call(
							this,
							`${bucketName}.s3`,
							'GET',
							`/${fileKey}`,
            ...
            */
              ...
              */
              const response = await store.listV2(qs as unknown as ListV2ObjectsQuery, {});
              ...
              */
              ...
              responseData = response.objects;
						...
						if (Array.isArray(responseData)) {
							responseData = responseData.filter(
								(e: OSS.ObjectMeta) => !e.name.endsWith('/') && e.size !== 0,
							);
							if (qs.limit) {
								responseData = responseData.splice(0, qs.limit as number);
							}
            ...
            */
              ...
              responseData = await store.put(`${path}${fileName || binaryPropertyData.fileName}`, binaryDataBuffer)
              ...
              */
              ...
              responseData = await store.put(`${path}${fileName}`, body)
